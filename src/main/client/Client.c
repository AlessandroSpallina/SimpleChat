...
-****************************************************************************/
/*Client -- Greco Alessio -- UniCT/Inf.Inf/Sistemi Opearativi/Prof.sa LLB-- */
/*-------------------------------Header-------------------------------------*/
#include <string.h>
#include <signal.h>
#include <stdlib.h>
...
int main (void){
	...
	switch (stoc.CMD){
		...
		case CONNECTED: {
			...
			argthead.socket_login = socket_login;
			argthead.socket_chat = socket_chat;
			argthead.socket_message = socket_message;
			if (pthread_create(&recmsg, NULL, ClientThreadMSG, (void*)&argthead) == -1){
